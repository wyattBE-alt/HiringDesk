...
// Resolve the signed-in user if a valid token is present, else null.
// Lets endpoints work anonymously but attribute data when the user is logged in.
...
async function searchJobs(query, location, numResults = 20, filters = {}) {
  ...
  const params = new URLSearchParams({
    ...
    date_posted: ["all", "today", "3days", "week", "month"].includes(filters.datePosted) ? filters.datePosted : "all"
  });
  ...
  return (data.data || []).slice(0, numResults).map((job) => ({
    id: job.job_id,
    title: job.job_title,
...
app.post("/api/analyze", upload.single("resumeFile"), async (req, res) => {
  try {
    ...
    const filters = {
      datePosted: (req.body.datePosted || "all").trim(),
      remoteOnly: req.body.remoteOnly === "true",
      employmentTypes: (req.body.employmentType || "").trim() || null
    };
...
<head>
  <meta charset="UTF-8">
  <title>PathAscent Candidate Report</title>
</head>
<body style="margin:0;padding:0;background:#f3efe5;font-family:'Segoe UI',sans-serif">
  <div style="max-width:860px;margin:32px auto;background:#fff;border-radius:20px;overflow:hidden;box-shadow:0 8px 40px rgba(0,0,0,0.10)">
    <div style="background:linear-gradient(135deg,#1f8f63,#2463eb);padding:32px 36px;color:white">
      <p style="margin:0 0 6px;font-size:0.76rem;letter-spacing:0.18em;text-transform:uppercase;opacity:0.8">PathAscent · Candidate Report</p>
    ...
    <div style="padding:24px 36px;border-top:1px solid #eee;margin-top:12px;color:#5f6873;font-size:0.82rem">
      Generated by PathAscent · ${new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}
...
app.post("/api/recruiter/rank", upload.array("resumeFiles", 100), async (req, res) => {
  try {
    ...
    // Persist ranked candidates ONLY for a signed-in recruiter, attributed to them
    // (their private history). Anonymous rankings are returned but never pooled —
...
app.post("/api/recruiter/past-match", async (req, res) => {
  try {
    const job = req.body;
    // Resurface only the signed-in recruiter's own prior candidates.
    const owner = await resolveUser(req);
    if (owner?.role !== "recruiter") {
      return res.json({ candidates: [], totalInDb: 0 });
    }
    const allPast = await getPastCandidates(owner.id);
    const totalInDb = await getCandidateCount(owner.id);
...
app.post("/api/recruiter/notify", async (req, res) => {
  try {
    ...
    await transporter.sendMail({
      ...
      to: recipient,
      subject: `PathAscent: Top ${candidates.length} Candidates for ${job?.title || "Open Role"}`,
      html: emailHtml
    });
...
app.get("/api/recruiter/talent-pool", requireAuth, requireRole("recruiter"), async (req, res) => {
  try {
    ...
    const candidates = await getTalentPool({ recruiterId: req.user.id, skills, minScore, limit: 100 });
    const totalInDb = await getCandidateCount(req.user.id);
...
app.listen(PORT, () => {
  console.log(`PathAscent running → http://localhost:${PORT}`);
